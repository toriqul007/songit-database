...
def task4():
    oldest_album = query('SELECT albums.title, MIN(albums.year_released) FROM albums', {})
...
def task6():
    album_list= query('SELECT albums.title, albums.year_released FROM albums', {})
...
def task14():
    ...
    for artist in artist_details:
        print(artist[0], artist[1], '(',artist[2],')', ' Albums: ', artist[4])
...
def task15and16():
    ...
    for album in select_album:
        print(album[0], '(',album[1],')', 'Songs:', album[2], '(Total song:', album[3], 'Total duration:', album[4] ,'seconds)')
